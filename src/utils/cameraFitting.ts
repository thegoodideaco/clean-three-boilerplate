export function calculateCameraDistance(planeWidth: number, planeHeight: number, camera: THREE.PerspectiveCamera, fitOutside: boolean): number {
...
export function getPixelScaleFromCameraDistance(cameraDistance: number, camera: THREE.PerspectiveCamera): number {
    const fov = camera.fov * ( Math.PI / 180 );
    const fovh = 2*Math.atan(Math.tan(fov/2) * camera.aspect);
